...
class SmsStrategyImpl(
    @Qualifier("smsService") private val smsSenderInputPort: MailSenderInputPort,
) : GenericRecordStrategy<MailCommand> {
    override suspend fun process(value: MailCommand): Result<Unit, Throwable> = value.toDomain()
        .andThen { transaction ->
            smsSenderInputPort.send(transaction)
        }

    override fun canProcess(record: GenericRecord): Boolean {
        return record is MailCommand && record.data.type == MailTypeCommand.SMS
    }
}
